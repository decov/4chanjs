        ...
        if (!Main.tid) {
            if (Config.filter) {
                filtered = Filter.exec(
                    thread, pi, document.getElementById('m' + tid), tid
                );
            }
...
Parser.addOekakiEditLink = function(pid, tid) {
    ...
    el.innerHTML = ' <a href="#" data-pid="' + pid + '" data-tid="' + tid + '" data-tip="Open in Tegaki" data-cmd="qr-painter-edit">Edit</a>';
...
Depager.onScroll = function() {
    if (document.documentElement.scrollHeight <= (Math.ceil(window.innerHeight + window.pageYOffset) + Depager.threshold)) {
...
Depager.renderNext = function() {
    var el, frag, i, j, k, threads, op, summary, cnt, reply, parseList, scroll, last_replies, boardDiv, pageId, data, isLastPage;
...
Depager.depage = function() {
    ...
    $.get('//a.4cdn.org/' + Main.board + '/catalog.json', {
        onload: Depager.onLoad, onerror: Depager.onError
    });
...
QuotePreview.show = function(link, post, remote) {
    var rect, postHeight, doc, docWidth, style, pos, quotes, i, j, qid, top, scrollTop, img, media;
...
ImageExpansion.fitWebm = function() {
    var imgWidth, imgHeight, maxWidth, maxHeight, ratio, left, cntEl, centerWidth, ofs, player, target;
...
ImageExpansion.onLoadStart = function(img, thumb) {
    var imgWidth, imgHeight, maxWidth, maxHeight, ratio, left, fileEl, cntEl, centerWidth, ofs, el;
...
ImageHover.showWebMDuration = function(el, thumb) {
    ...
    [w, h] = $.fit(el.videoWidth, el.videoHeight, window.innerWidth - aabb.right - 20, window.innerHeight);
...
QR.onCTMouseOver = function() {
    QR.onCTMouseOut();
    QR.ctTipTimeout = setTimeout(Tip.show, Tip.delay, QR.captchaWidgetCnt, 'Verification not required for your next post.');
};
...
QR.show = function(tid) {
    var i, j, cnt, postForm, form, qrForm, fields, row, spoiler, file, el, el2, placeholder, qrError, cookie, mfs;
    ...
    form.innerHTML =
        ((mfs = $.byName('MAX_FILE_SIZE')[0]) ? ('<input type="hidden" value="' + mfs.value + '" name="MAX_FILE_SIZE">') : '')
        ...
        if (fields[i].id == 'captchaFormPart') {
            ...
            if (placeholder == 'Password' || placeholder == 'Spoilers') {
                ...
                if (el.nodeName == 'INPUT' || el.nodeName == 'TEXTAREA') {
                    ...
                    if ((cookie = localStorage.getItem('4chan_flag_' + Main.board)) && (el2 = $.qs('option[value="' + cookie + '"]', el))) {
...
QR.submit = function(force) {
    ...
    if (formdata.entries && formdata.delete && (!formdata.get('upfile') || !formdata.get('upfile').size)) {
...
ReplyHiding.save = function() {
    ...
    for (i in this.hiddenR) {
        localStorage.setItem('4chan-hide-rr-' + Main.board, JSON.stringify(this.hiddenR));
...
ThreadWatcher.onClick = function(e) {
    ...
    if (t.hasAttribute('data-id')) {
        ThreadWatcher.toggle(
            t.getAttribute('data-id'), t.getAttribute('data-board')
        );
...
ThreadExpansion.fetch = function(tid) {
	ThreadExpansion.fetchXhr = $.get(
		...
		{
			onload: function() {
				var i, p, n, frag, thread, tail, posts, msg, metacap, expmsg, summary, abbr;
...
ThreadUpdater.onScroll = function() {
	if (ThreadUpdater.hadAuto && (document.documentElement.scrollHeight <= (Math.ceil(window.innerHeight + window.pageYOffset)) && !document[ThreadUpdater.hidden])) {
...
ThreadUpdater.update = function(full) {
	...
	url = '//a.4cdn.org/' + Main.board + '/thread/' + Main.tid + (isTail ? '-tail' : '') + '.json';
...
ThreadUpdater.onload = function() {
	var i, state, self, nodes, thread, newposts, frag, lastrep, lastid, op, doc, autoscroll, count, fromQR, lastRepPos;
	...
	if (this.status == 200) {
		...
		if (count) {
			...
			autoscroll = (
				Config.autoScroll && document[self.hidden] && doc.scrollHeight == Math.ceil(window.innerHeight + window.pageYOffset)
			);
...
Filter.load = function() {
	var i, j, f, rawFilters, rawPattern, fid, regexEscape, regexType, wordSepS, wordSepE, words, inner, regexWildcard, replaceWildcard, tmp, boards, pattern, match;
...
SWFEmbed.embedIndex = function(e) {
	var el, cnt, header, icon, backdrop, width, height, cntWidth, cntHeight, maxWidth, maxHeight, docWidth, docHeight, margins, headerHeight, fileName, ratio;
...
Main.onThreadMouseOver = function(e) {
	...
	if (Config.quotePreview && $.hasClass(t, 'quotelink') && !$.hasClass(t, 'deadlink') && !$.hasClass(t, 'linkfade')) {
		...
		(t.hasAttribute('data-md5') && !$.hasClass(t.parentNode, 'deleted')) || (t.href && !$.hasClass(t.parentNode, 'fileText') && /(i\.4cdn|is\.4chan)\.org\/[a-z0-9]+\/[0-9]+\.(gif|jpg|png|webm)$/.test(t.href)))
	) {
		ImageHover.show(t);
...
Main.onThreadMouseOut = function(e) {
	...
	} else if (Config.imageHover && (t.hasAttribute('data-md5') || (t.href && !$.hasClass(t.parentNode, 'fileText') && /(i\.4cdn|is\.4chan)\.org\/[a-z0-9]+\/[0-9]+\.(gif|jpg|png|webm)$/.test(t.href)))
	) {
		ImageHover.hide();
